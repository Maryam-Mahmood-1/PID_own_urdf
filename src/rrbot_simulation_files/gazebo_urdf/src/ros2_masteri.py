...
class MotorController(Node):
    def __init__(self):
        ...
        self.position_constraints = [(-30, 30)] * self.num_joints  # example constraints in degrees
        self.initial_angles = [0] * self.num_joints  # example initialization values

        # PID parameters
        self.proportional_gain = [0.6, 0.5, 0.1, 1.6, 2.0, 3.0, 2.0]
        #                        [141,   142,   144,   143,  146,   145,   147]
        self.derivative_gain = [0.003, 0.003, 0.003, 0.002, 0.002, 0.0001, 0.002]
        self.integral_gain = [0.000196, 0.000196, 0.000196, 0.0001, 0.000196, 0.00009, 0.0003]
    ...
    def calculate_pid_speeds(self):
        ...
        for i in range(self.num_joints):
            if abs(error[i]) > self.acceptable_error:
                ...
                # Convert to dps
                self.apply_joint_velocities[i] = int(np.clip(self.calc_vel_rad[i]*57.29, -50, 50))
            else:
        ...
        self.get_logger().info(f"Publishing Derivative Error: {[d * (s / 57.29) for d, s in zip(self.derivative_gain, self.current_speeds)]}")
